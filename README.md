...
주요 결정이나 변경 사항이 생긴 이유를 남겨둡니다.

### API-Football → football-data.org 교체 (2026-05-12)

초기에는 **API-Football**(api-sports.io)을 사용했습니다. 그런데 무료 플랜이 2022~2024 시즌까지만 지원하여, 실제 서비스에서 항상 지난 시즌 데이터만 보이는 문제가 있었습니다. 현재 진행 중인 2025/26 시즌 데이터를 보여주려면 유료 플랜이 필요했습니다.

**football-data.org**로 교체한 이유:
- 무료 플랜에서 **현재 시즌 데이터** 제공
- EPL, 라리가, 분데스리가, 세리에A, 리그앙 등 주요 리그 지원
- 응답 구조가 명확하고 문서화가 잘 되어 있음

교체 과정에서 리그 ID 체계가 달라져 (예: EPL 39 → 2021) 프론트엔드와 백엔드 전체에서 ID를 일괄 변경했습니다. 또한 팀 검색과 선수 검색은 football-data.org 무료 플랜에서 지원하지 않아, 팀 검색은 지원 리그의 팀 목록을 서버 시작 시 캐싱해 이름으로 필터링하는 방식으로 대체했습니다.

### 날짜별 경기 조회 및 KST 시간대 처리 (2026-05-12)

경기 일정을 오늘만 보여주는 구조에서, 날짜를 직접 선택할 수 있도록 개선했습니다. 달력 팝업 UI를 추가하여 원하는 날짜의 경기를 바로 확인할 수 있게 했습니다.

이 과정에서 **UTC와 KST 시간대 차이** 문제가 발생했습니다. football-data.org는 UTC 기준으로 경기 날짜를 반환하는데, 한국(KST, UTC+9)에서는 UTC 기준 날짜와 실제 체감 날짜가 최대 하루 차이납니다. 예를 들어 라리가 경기가 UTC 기준 5월 12일 17:00이면 KST로는 5월 13일 02:00입니다. 이를 그대로 표시하면 한국 사용자 기준으로 날짜가 하루 당겨져 보이게 됩니다.

백엔드에서 UTC 날짜를 KST로 변환한 뒤 필터링하도록 수정하여 해결했습니다.

또한 football-data.org의 특이한 동작으로, `dateFrom`과 `dateTo`를 같은 날짜로 지정하면 데이터를 반환하지 않는 버그가 있습니다. 이를 `dateTo`를 하루 늘려 쿼리하고 KST 기준으로 재필터링하는 방식으로 우회했습니다.
